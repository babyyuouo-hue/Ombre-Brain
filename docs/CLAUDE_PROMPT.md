...
`hold(source_content=...)` 与 `grow(content=共享原文, items=[...])` 仍会把原文存进不可变原文层，但**我没有通用回读工具**。只有小宇明确把某个桶的 `tags` 或 `domain` 改为 `letter`，`letter_read` 才会逐字返回那一个桶活动原文引用覆盖的范围；不要自行给桶加这个标签。
